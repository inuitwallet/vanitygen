...
/* Duplicate thread loop with controlled loop to gather system info rather than find the address */

void *
vg_thread_system_info(void *arg)
{
	unsigned char hash_buf[128];
	unsigned char *eckey_buf;
	unsigned char hash1[32];

	int i, c, len, output_interval, n;
	int hash_len;
